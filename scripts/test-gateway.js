 ...
 * 
 * Usage:
 *   node test-gateway.js <gateway-url> [commit-count]
 * 
 * Arguments:
...
// Import required cryptographic primitives from @unicitylabs/commons
const { SigningService } = require('@unicitylabs/commons/src/signing/SigningService');
const { DataHasher } = require('@unicitylabs/commons/src/hash/DataHasher');
const { HashAlgorithm } = require('@unicitylabs/commons/src/hash/HashAlgorithm');
const { DataHash } = require('@unicitylabs/commons/src/hash/DataHash');
const { Authenticator } = require('@unicitylabs/commons/src/api/Authenticator');
const { RequestId } = require('@unicitylabs/commons/src/api/RequestId');
...
function createDataHashFromHex(hexString) {
  return DataHash.fromBytes(Buffer.from(hexString, 'hex'), HashAlgorithm.SHA256);
}

// Generate a random DataHash
async function generateRandomDataHash() {
  const hasher = new DataHasher(HashAlgorithm.SHA256);
  hasher.update(crypto.randomBytes(32));
...
async function createRandomCommitment(keypair) {
  ...
  const requestId = await RequestId.create(keypair.publicKey, stateHash.imprint);
...
async function submitCommitment(gatewayUrl, commitment) {
  try {
...
async function getInclusionProof(gatewayUrl, requestId) {
  try {
    // Ensure the URL doesn't have trailing slashes
    const normalizedUrl = gatewayUrl.endsWith('/') ? gatewayUrl.slice(0, -1) : gatewayUrl;
    const requestUrl = `${normalizedUrl}/`;
...
async function main() {
  ...
  if (args.length < 1) {
    console.error('Usage: node test-gateway.js <gateway-url> [commit-count]');
    process.exit(1);
  }
  ...
  try {
    ...
    console.warn(`⚠️ WARNING: Invalid URL format: ${error.message}`);
