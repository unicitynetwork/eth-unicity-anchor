...
set -e
# -----------------------------------------------------------------------
# Ethereum Unicity Anchor Gateway Synchronization Test Runner
#
# This script:
# 1. Starts a local Anvil Ethereum node
# 2. Compiles and deploys the AggregatorBatches contract
# 3. Runs the Gateway synchronization test that verifies:
#    - SMT synchronization on startup
#    - Periodic processing of new batches
#    - Handling mismatched hashroots between local and on-chain data
#    - Concurrent batch processing from multiple gateways
...
LOGFILE="sync-test-output.log"
echo "============================================" > $LOGFILE
echo "Gateway Synchronization Test Run - $(date)" >> $LOGFILE
echo "============================================" >> $LOGFILE
...
# Run the gateway synchronization test specifically
log "Running Gateway Synchronization Tests..."
cd ts-client
log "Contract address for tests: $CONTRACT_ADDRESS"

# Create synchronization test log file
SYNC_TEST_LOG="sync-test-output.log"
log "Synchronization test output will be written to $SYNC_TEST_LOG"
...
echo -e "\n--------- GATEWAY SYNC TEST RESULTS (exit code: $TEST_EXIT_CODE) ---------"
if grep -q "FAIL" "$SYNC_TEST_LOG"; then
  echo -e "\n❌ TEST FAILURES DETECTED!\n"
  grep -A10 "FAIL" "$SYNC_TEST_LOG" | head -20
  echo -e "\n"
  grep -A5 "● Gateway SMT Synchronization Tests" "$SYNC_TEST_LOG" | head -10
  echo -e "\nSee $SYNC_TEST_LOG for full details"
  TEST_EXIT_CODE=1
elif grep -q "Test Suites: .* failed" "$SYNC_TEST_LOG"; then
  echo -e "\n❌ TEST SUITES FAILED!\n"
  grep -A5 "Test Suites:" "$SYNC_TEST_LOG"
  echo -e "\nSee $SYNC_TEST_LOG for full details"
  TEST_EXIT_CODE=1
else
  echo -e "\n✅ ALL TESTS PASSED\n"
  grep -A3 "Test Suites:" "$SYNC_TEST_LOG" || echo "No test summary found"
fi
echo -e "---------------------------------------------------\n"

# Check if anything was written to the log
if [ ! -s "$SYNC_TEST_LOG" ]; then
  log "⚠️ Warning: Synchronization test output log is empty!"
...
if grep -q "FAIL " "$SYNC_TEST_LOG" || grep -q "Test Suites:.*[1-9][0-9]* failed" "$SYNC_TEST_LOG" || grep -q "Tests:.*[1-9][0-9]* failed" "$SYNC_TEST_LOG"; then
  ...
  grep -A10 "FAIL " "$SYNC_TEST_LOG" | head -10
  
  # Always output full log on failure
  cat "$SYNC_TEST_LOG" >> ../$LOGFILE
  
  # Set exit code to fail
  TEST_EXIT_CODE=1
else
  # No failures detected in the logs, show summary
  log "Synchronization test output summary (see $SYNC_TEST_LOG for details):"
  grep -E "(PASS|✓)" $SYNC_TEST_LOG | tail -5 >> ../$LOGFILE
  grep -A3 "Test Suites:" "$SYNC_TEST_LOG" >> ../$LOGFILE 2>/dev/null
  
  # If no match was found, show the last 20 lines
  if [ $? -ne 0 ]; then
    log "No test results found in log, showing last 20 lines:"
    tail -20 $SYNC_TEST_LOG >> ../$LOGFILE
  fi
fi

# Set final exit code based on test output
if [ $TEST_EXIT_CODE -ne 0 ] || grep -q "FAIL" "$SYNC_TEST_LOG" || grep -q "Test Suites:.*[1-9][0-9]* failed" "$SYNC_TEST_LOG" || grep -q "Tests:.*[1-9][0-9]* failed" "$SYNC_TEST_LOG"; then
  # Direct console output for better visibility
  echo -e "\n❌ TESTS FAILED! Check the logs for details."
  
  # Log to file
  log "❌ Gateway sync tests failed!"
  
  # Extract the number of failed tests for better reporting
  FAILED_TESTS=$(grep -o "Tests:.*[1-9][0-9]* failed" "$SYNC_TEST_LOG" | grep -o "[1-9][0-9]* failed" || echo "Unknown failures")
  ...
  echo -e "\nFailed test details:"
  grep -B1 -A5 "● .* ›" "$SYNC_TEST_LOG" | grep -v "^--$" | tee -a ../$LOGFILE
  ...
  echo -e "\n✅ ALL GATEWAY SYNC TESTS PASSED SUCCESSFULLY!"
  
  # Log to file
  log "✅ Gateway synchronization tests completed successfully!"
  FINAL_EXIT_CODE=0
fi
...
if [ "$FINAL_EXIT_CODE" -ne 0 ]; then
  ...
  echo -e "===========================================\n"
  echo -e "Check ts-client/sync-test-output.log for full details"
  # Also log to file
